...
- [ ] **Step 1: Define `StateMachine` trait.** Associated types: `State: Send + Clone + 'static`, `Input: Message<Result = TransitionResult<Self::State>> + Send + 'static`. Methods: `fn transition(&self, state: &Self::State, input: &Self::Input) -> Result<Self::State, String>`, `fn on_enter(&mut self, old: &Self::State, new: &Self::State, ctx: &mut Context<FsmActor<Self>>)` (default no-op). Bound: `Send + 'static`.
