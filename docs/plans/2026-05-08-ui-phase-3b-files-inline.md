...
- [x] **T6.** Implement `<AttachmentVoiceNote>` per spec + `VoiceNotePlayer` single-instance coordinator. **Browser tests:** AG-5, AG-6. **Verify:** `just test-browser`. *(Landed: card-local `<audio>` for playback, `AudioContext.decodeAudioData` for waveform peaks with a flat baseline fallback, shared `VoiceNotePlayer` context with `claim` / `release_if_active` semantics. AG-5 covered by `phase_3b_voice_note::voice_note_card_renders_spec_surface`; AG-6 covered by `second_claim_pauses_first_via_shared_player`.)*
