...
- [ ] **T2.** Add `ClientHandle::upload_attachment(channel_id, filename, mime, bytes) -> Result<AttachmentHandle>` that wraps the existing iroh blob store, content-addresses the bytes, and returns `{ hash, size_bytes }`. Add `send_attachment_message(channel_id, AttachmentMeta { hash, filename, mime, size, width, height }, opt_caption)`. Keep `share_file_inline` but mark `#[deprecated(note = "…")]`. **Tests:** `upload_attachment_returns_hash`, `upload_attachment_dedupes_by_hash`, `send_attachment_round_trips_through_state`, `send_attachment_carries_dimensions`. **Verify:** `just test-client`.
