...
- [ ] **T4.** Create `crates/web/src/components/composer/` module + `placeholders.rs` with `placeholder_for(...)`. Pure function with 4 unit tests (text / letter / offline / no-channel). Wire into `components/mod.rs`. **Verify:** `cargo test -p willow-web`.
