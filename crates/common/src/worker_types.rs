...
impl PartialEq for WorkerResponse {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (
                WorkerResponse::SyncBatch { events: a, .. },
                WorkerResponse::SyncBatch { events: b, .. },
            ) => {
...
impl WorkerResponse {
    ...
    pub fn last_synced_hash(&self) -> Option<willow_state::EventHash> {
        match self {
            ...
            // Feedback replies stream no DAG events, so they carry no
            // end-of-stored-events hash.
            WorkerResponse::Denied { .. }
            | WorkerResponse::FeedbackOk { .. }
            | WorkerResponse::FeedbackErr { .. } => None,
        }
    }
}
